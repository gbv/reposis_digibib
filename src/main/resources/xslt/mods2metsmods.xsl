...
<xsl:stylesheet version="3.0"
  ...
  <xsl:template match="/mycoreobject">
    ...
    <xsl:for-each select="structure/derobjects/derobject">
      ...
      <xsl:variable name="metsChild" select="
        $derivateIFSXML/mcr_directory/children/child[@type='file' and name/text()='mets.xml']
      " />
            ...
            <mets:mdWrap MIMETYPE="text/xml" MDTYPE="OTHER" OTHERMDTYPE="DVRIGHTS">
              <mets:xmlData>
                <dv:rights xmlns:dv="http://dfg-viewer.de/">
                  ...
                  <dv:license>
                    <xsl:choose>
                      <xsl:when test="$object/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:accessCondition[@type='use and reproduction']">
                        <xsl:variable name="licenseClass" select="'mir_licenses'" />
                        <xsl:variable name="licenseId" select="
                          substring-after($object/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:accessCondition[@type='use and reproduction']/@xlink:href, '#')
                        " />
                        <xsl:variable name="licenseCategory" select="
                          document('classification:metadata:-1:children:mir_licenses')/mycoreclass//category[@ID=$licenseId]
                        " />
                        <xsl:choose>
              ...
              <mets:xmlData>
                <dv:links xmlns:dv="http://dfg-viewer.de/">
                  <xsl:variable name="ppn" select="
                    substring-after($object/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:identifier[@type='uri'], ':ppn:')
                  " />
                  <xsl:if test="$ppn">
                    <dv:reference>
                      <xsl:variable name="catalogURL" select="
                        concat(substring-before($MIR.DFGViewer.DV.OPAC.CATALOG.URL, '{PPN}'), $ppn , substring-after($MIR.DFGViewer.DV.OPAC.CATALOG.URL, '{PPN}'))
                      " />
                      <xsl:variable name="uriResolved" select="
                        document(concat($catalogURL,'?format=xml'))//rdf:Description[@rdf:about=normalize-space($catalogURL)]/*[local-name() = 'page']/@rdf:resource
                      " />
  ...
  <xsl:template match="mets:fileSec">
    ...
    <xsl:copy>
      <xsl:copy-of select="@*" />
      <xsl:variable name="imageFiles" select="
        mets:fileGrp[@USE='MASTER']/mets:file[ends-with(mets:FLocat/@xlink:href, '.jpg')
        or ends-with(mets:FLocat/@xlink:href, '.png')
        or ends-with(mets:FLocat/@xlink:href, '.tiff')]
      " />
      <xsl:variable name="altoFiles" select="
        mets:fileGrp[@USE='ALTO']/mets:file[ends-with(mets:FLocat/@xlink:href, '.xml')
        and starts-with(mets:FLocat/@xlink:href, 'alto/')]
      " />
      <mets:fileGrp USE="THUMBS">
        <xsl:for-each select="$imageFiles">
          ...
          <mets:file ID="{concat('THUMBS_', $id)}" MIMETYPE="image/jpeg">
            <mets:FLocat
              xmlns:xlink="http://www.w3.org/1999/xlink" LOCTYPE="URL"
