...
>
  ...
  <xsl:template match="/mycoreobject[contains(@ID,'_mods_')]" mode="objectActions" priority="2">
    ...
    <!-- BEGIN LEOPARD SPECIFIC CHANGE -->
    <xsl:param name="manageAccessKeys" select="key('rights', $id)/@manageAccessKeys" />
     <!-- END LEOPARD SPECIFC CHANGE -->
    ...
    <div class="btn-group d-flex">
      ...
      <div class="btn-group w-100">
        ...
        <ul class="dropdown-menu dropdown-menu-right">
          ...
          <!-- BEGIN LEOPARD SPECIFC CHANGE -->
          <xsl:if test="$manageAccessKeys and contains($MCR.ACL.AccessKey.Strategy.AllowedObjectTypes, 'mods')">
            <li>
              <a role="menuitem" tabindex="-1"
                href="{$WebApplicationBaseURL}access-key-manager?objectId={@ID}"
                class="dropdown-item">
                <xsl:value-of select="i18n:translate('mir.accesskey.manage')" />
              </a>
            </li>
      ...
      <div class="dropdown float-right options">
        <div class="btn-group">
          ...
          <ul class="dropdown-menu dropdown-menu-right">
            ...
            <!-- BEGIN LEOPARD SPECIFC CHANGE -->
            <xsl:if test="key('rights', $deriv)/@manageAccessKeys and contains($MCR.ACL.AccessKey.Strategy.AllowedObjectTypes, 'derivate')">
              <li>
                <a role="menuitem" tabindex="-1" class="dropdown-item"
                  href="{$WebApplicationBaseURL}access-key-manager?objectId={$deriv}">
                  <xsl:value-of select="i18n:translate('mir.accesskey.manage')" />
                </a>
              </li>
            </xsl:if>
            <!-- END LEOPARD SPECIFC CAHNGE-->
          </ul>
        </div>
      </div>
    </xsl:if>
