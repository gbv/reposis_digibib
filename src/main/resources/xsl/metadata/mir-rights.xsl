...
>
  ...
  <xsl:variable name="manageReadAccessKeys" select="'manage-read-access-keys'" />
  <xsl:variable name="manageWriteAccessKeys" select="'manage-write-access-keys'" />
  ...
  <xsl:template name="check-rights">
    ...
    <right id="{$id}">
      ...
      <!-- BEGIN LEOPARD SPECIFIC CHANGE -->
      <xsl:call-template name="check-access-key-manage-rights">
        <xsl:with-param name="id" select="$id" />
      </xsl:call-template>
      <!-- END LEOPARD SPECIFC CHANGE -->
    </right>
  ...
  <!-- BEGIN LEOPARD SPECIFIC CHANGE -->
  <xsl:template name="check-access-key-manage-rights">
    <xsl:param name="id" />
    <xsl:if test="acl:checkPermission($id,$manageWriteAccessKeys) or acl:checkPermission($id,$manageReadAccessKeys)">
      <xsl:attribute name="manageAccessKeys" />
    </xsl:if>
  </xsl:template>
  <!-- END LEOPARD SPECIFC CHANGE -->
</xsl:stylesheet>
