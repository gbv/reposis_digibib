...
/**
 * Event handler for contact email events.
 * This class extends {@link ContactRequestEventHandlerBase} and handles events related to contact requests,
 * specifically sending email notifications or collecting information when a contact request is created.
 */
    ...
    protected void handleContactRequestCreated(MCREvent evt, ContactRequestDto contactRequestDto) {
        if (SEND_REQUEST_CONFIRMATION_EMAIL) {
            MCRJobQueueManager.getInstance().getJobQueue(EmailJobAction.class)
                .add(EmailJobAction.createRequestConfirmationJob(contactRequestDto.getId()));
        }
        if (SEND_NEW_REQUEST_INFO_ENAIL) {
            MCRJobQueueManager.getInstance().getJobQueue(EmailJobAction.class)
                .add(EmailJobAction.createNewRequestInfoJob(contactRequestDto.getId()));
        }
