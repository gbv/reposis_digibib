...
/**
 * Seeks related contact on orcid.org.
 */
public class ContactInfoOrcidCollector implements ContactInfoCollector {
    ...
    @Override
    public List<ContactInfo> collect(MCRObject object) {
    ...
    /**
     * Retrieves and returns list over ORCiD elements.
     *
     * @param object object
     * @return list over ORCiD elements
     */
    ...
    /**
     * Seeks ORCiD, maps and returns list over contact elements.
     *
     * @param orcid ORCiD
     * @return list over contact info elements
     */
    protected List<ContactInfo> processOrcid(String orcid) {
        ...
        if (orcidUser != null) {
            ...
            return emails.stream().map(m -> new ContactInfo(name, m, "orcid", orcid)).toList();
        } else {
            try {
                return fetchEmailsFromPublicApi(orcid).stream().map(m -> new ContactInfo(name, m, "orcid", orcid))
                    .toList();
            } catch (Exception e) {
                LOGGER.warn(e);
            }
