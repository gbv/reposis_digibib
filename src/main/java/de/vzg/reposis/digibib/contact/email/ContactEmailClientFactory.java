...
/**
 * Factory for {@link ContactEmailClient}.
 */
public class ContactEmailClientFactory {

    private static final String CONF_PREFIX = ContactConstants.CONF_PREFIX + "EmailClient.";
    ...
    private static ConcurrentHashMap<String, ContactEmailClient> sessions = new ConcurrentHashMap<>();

    /**
     * Returns and creates if not exists {@link ContactEmailClient} for name.
     *
     * @param name name
     * @return client
     */
    public static ContactEmailClient getInstance(String name) {
        return sessions.computeIfAbsent(name, n -> new ContactEmailClient(createSession(n)));
    }
