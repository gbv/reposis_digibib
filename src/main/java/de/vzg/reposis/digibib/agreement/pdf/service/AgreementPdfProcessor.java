...
import de.vzg.reposis.digibib.agreement.model.FormField;

/**
 * Utility class for processing and filling PDF forms for agreements.
 * <p>
 * This class provides functionality to populate fields in a {@link PDAcroForm} using data from an
 * {@link AgreementFormData} object. It supports text fields, checkboxes, and radio buttons.
 * <p>
 * The class also ensures that text fields use a default font and appearance (Helvetica, size 10).
 */
public class AgreementPdfProcessor {
     ...
     * @param formData the {@link AgreementFormData} containing field names, types, and values
     * @throws IOException if an I/O error occurs while setting the field values
     * @throws AgreementException if a required PDF field is missing or a value is invalid
     */
    public void fillForm(PDAcroForm acroForm, AgreementFormData formData) throws IOException {
        LOGGER.debug("Filling acro form with agreement form data...");
            ...
            if (pdField != null) {
                ...
                switch (field.getType()) {
                    ...
                    case RADIO:
                        ...
                        if (radioButton.getExportValues().contains(value)) {
                            pdField.setValue(value);
                        } else {
                            throw new AgreementException("Radio '" + field.getName() + "' has no value '" + value);
                        }
                        break;
                }
            } else {
                throw new AgreementException("PDF field '" + field.getName() + "' not found");
            }
        }
